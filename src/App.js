...
import TabItems from './components/TabItem/TabItem'
import ThumbItems from './components/ThumbNailItems/ThumbnailList'
...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.secChange, 1000)
  }
  ...
  FilterTabImages = () => {
    ...
    const FilterData = imagesList.filter(
  ...
  render() {
    const {activeId, ImgUrl, score, time, isTrue} = this.state
    const FilterData = this.FilterTabImages()
    return (
      <div className="Main-container">
        <MatchGame score={score} time={time} />

        <div className="container-2">
          {!isTrue && (
            <div className="arrange-list">
              <img src={ImgUrl} alt="match" className="large-img-size" />
              ...
              <ul className="ul2-style">
                {FilterData.map(eachItem => (
                  <ThumbItems
                    ThumbDetails={eachItem}
                    MatchGameClick={this.MatchGameClick}
                    key={eachItem.id}
                  />
                ))}
              </ul>
          ...
          {isTrue && (
            <div className="container-won-page">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
                alt="trophy"
                className="img-cup"
              />
              <p className="para-won">YOUR SCORE</p>
              <p className="para-won-score">{score}</p>
              <button
                className="button-won-style"
                type="button"
